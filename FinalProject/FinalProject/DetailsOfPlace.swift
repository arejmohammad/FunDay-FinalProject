...
class DetailsOfPlace: UIViewController {
    ...
    @IBAction func savetofav(_ sender: Any) {
        if Auth.auth().currentUser != nil {
            let dialogMessage = UIAlertController(title: "saved".loclized(), message: "this place saved to favorite list", preferredStyle: .alert)
            let ok = UIAlertAction(title: "OK".loclized(), style: .default, handler: nil)
            dialogMessage.addAction(ok)
            self.present(dialogMessage, animated: true, completion: nil)
            print("Saved")
        }else {
            let dialogMessage = UIAlertController(title: "Ops", message: "you shold sign in to save in favorite list", preferredStyle: .alert)
            let ok = UIAlertAction(title: "OK".loclized(), style: .default, handler: nil)
            dialogMessage.addAction(ok)
            self.present(dialogMessage, animated: true, completion: nil)
